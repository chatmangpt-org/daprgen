...
ENV VIRTUAL_ENV /opt/daprgen-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/daprgen/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/daprgen/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/daprgen/ && touch src/daprgen/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/daprgen/
...
# Expose the app.
ENTRYPOINT ["/opt/daprgen-env/bin/daprgen"]
CMD []
